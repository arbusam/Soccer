...
class SessionReplayTests(unittest.TestCase):
    def test_timeline_latest_and_nearest(self):
        events = [{"time": 0.1, "value": 1}, {"time": 0.3, "value": 2}]
        timeline = EventTimeline(events, "time")
        self.assertIsNone(timeline.latest(0.05))
        self.assertEqual(timeline.latest(0.2)["value"], 1)
        self.assertEqual(timeline.nearest(0.29, 0.02)["value"], 2)
        self.assertIsNone(timeline.nearest(0.2, 0.05))

    def test_loads_session_and_converts_legacy_field_order(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            session = load_recorded_session(directory)
            tokens = game_event_tokens(session.game_events[0])
            self.assertEqual(tokens[:3], ["10", "20", "30"])
            self.assertFalse(session.detection_events[0]["detected"])
