...
**How it works:**
- `speed` is in mm/s. Direction and yaw use a heading frame where `0` = forward and `90` = right. `movement.py` converts that to the mecanum wheel basis with `local_direction = direction - yaw - 135` (degrees).
...
- With diameter 50 mm: `mmps_to_rpm ≈ 0.382`. Example: speed 500, `direction = yaw = 0` gives `local_direction = -135°`, which corresponds to local forward in the current wheel basis.
