...
Defaults to the Intel Arc GPU via PyTorch XPU (same setup as training/train.py).
    ...
    if _is_xpu(device):
        if not hasattr(torch, "xpu") or not torch.xpu.is_available():
            raise RuntimeError(
                ...
                "  /home/arhan/training/.venv/bin/python training/train_detect.py ...\n"
                "or pass --device cpu"
            )
...
def export_onnx(weights: Path, export_dir: Path, imgsz: int) -> Path:
    ...
    meta.setdefault(
        "names",
        {0: "Ball", 1: "Blue Goal", 2: "Yellow Goal", 3: "Bot"},
    )
